...
Nota: Compilar usando: gcc memo.c -lm
...
int existeProceso(int idProceso, int tam);                                              // Verifica si el proceso ya esta en memoria
...
int LeerProcesos(int idProceso,int numpags, char a[30]){    
    ...
    while (feof(flujo)==0){                                                             // Inicia la lectura del archivo
        ...
        if (existeProceso(idProceso, numpags) && (numpags != -1))                       // Verificamos si el proceso no esta en memoria
...
int existeProceso(int idProceso, int tam)                                               // Recibe el proceso a buscar
{
    ...
    for (int i = 0; i < 16; ++i)                                                        // Recorre la memoria real
    {
        ...
        {
            if (tam != -1)                                                              // Solo notifica en inserciones
